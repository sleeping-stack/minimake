...
void parse_target_line(const char *str, Target_block *tb_arr, int tb_count)
{
    ...
    *colon = '\0';
    ...
    while (*current && tb_arr[tb_count].dep_count < MAX_DEP_NUMBERS)
    {
        ...
        size_t copy_len = token_len < sizeof(token_raw) - 1 ? token_len : sizeof(token_raw) - 1;
        ...
        while (*p && tb_arr[tb_count].dep_count < MAX_DEP_NUMBERS)
        {
            ...
            if (wlen > 0)
            {
                ...
                safe_copy(tb_arr[tb_count].dep_arr[tb_arr[tb_count].dep_count],
                          sizeof(tb_arr[tb_count].dep_arr[tb_arr[tb_count].dep_count]), word);
...
{
    if (tb_count < 0 || tb_count >= MAX_BLOCK_NUMBERS) return;
    if (cmd_count < 0 || cmd_count >= MAX_LINE_NUMBERS) return;
    // 去掉行尾换行
    ...
    safe_copy(tb_arr[tb_count].commands[cmd_count], sizeof(tb_arr[tb_count].commands[cmd_count]),
              cmd_exp);
}
...
int parse_check(Target_block *tb_arr, int tb_count)
{
    ...
    if (has_duplicate_target(tb_arr, tb_count) == 1) has_error = 1;
    ...
    for (int i = 0; i < tb_count; i++)
    {
        for (int j = 0; j < tb_arr[i].dep_count; j++)
        {
            if (is_dep_invaild(tb_arr, tb_count, tb_arr[i].dep_arr[j]) == 1) has_error = 1;
        }
    }
    ...
    if (has_error == 0)
        ...
        printf(
            "Please make sure the makefile has no grammar mistake so that the "
